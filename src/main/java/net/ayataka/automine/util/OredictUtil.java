...
public class OredictUtil {
    ...
    public static boolean isLog(final Block block) {
        return Arrays.stream(getOreNames(block)).anyMatch(s -> s.toLowerCase().startsWith("log"));
    }

    public static boolean isLeaf(final Block block) {
        return Arrays.stream(getOreNames(block)).anyMatch(s -> s.equalsIgnoreCase("treeLeaves"));
    }
}
